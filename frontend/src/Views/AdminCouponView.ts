...
export class AdminCouponView {
  private listView = document.getElementById("coupon-list-view");
  private formView = document.getElementById("coupon-form-view");
  private tableBody = document.getElementById("coupon-table-body");
  private btnShowAdd = document.getElementById("btn-show-add");
  private btnBackList = document.getElementById("btn-back-list");
  private btnSave = document.getElementById("btn-save-coupon");

  public currentEditId: string | null = null;

  constructor() {
    // Ép ẩn form khi vừa load trang
  ...
  getFormData() {
    return {
      code: (document.getElementById("cpn-code") as HTMLInputElement)?.value
        .trim()
        .toUpperCase(),
      discountType: (document.getElementById("cpn-type") as HTMLSelectElement)
        ?.value,
      discountValue: parseInt(
        (document.getElementById("cpn-value") as HTMLInputElement)?.value ||
          "0",
      ),
      minOrderAmount: parseInt(
        (document.getElementById("cpn-min-order") as HTMLInputElement)?.value ||
          "0",
      ),
      usageLimit: parseInt(
        (document.getElementById("cpn-limit") as HTMLInputElement)?.value ||
          "0",
      ),
      expiryDate: (document.getElementById("cpn-expiry") as HTMLInputElement)
        ?.value,
      status: (document.getElementById("cpn-status") as HTMLSelectElement)
        ?.value,
    };
  }
  ...
  fillForm(data: any, id: string) {
    ...
    (document.getElementById("cpn-code") as HTMLInputElement).value =
      data.code || "";
    (document.getElementById("cpn-type") as HTMLSelectElement).value =
    ...
    // Format ngày giờ cho input type="datetime-local"
    if (data.expiryDate) {
      const dateObj = new Date(data.expiryDate);
      const formattedDate = dateObj.toISOString().slice(0, 16);
      (document.getElementById("cpn-expiry") as HTMLInputElement).value =
        formattedDate;
    }

    (document.getElementById("cpn-status") as HTMLSelectElement).value =
      data.status || "Active";

    // Việt hóa tiêu đề & nút bấm khi SỬA
    const formTitle = document.getElementById("form-title");
    if (formTitle) formTitle.innerText = "Sửa Mã giảm giá";
    if (this.btnSave)
  ...
  clearForm() {
    ...
    // Việt hóa tiêu đề & nút bấm khi THÊM MỚI
    const formTitle = document.getElementById("form-title");
    if (formTitle) formTitle.innerText = "Thêm Mã giảm giá mới";
    if (this.btnSave)
      this.btnSave.innerHTML = `<i class="fa-solid fa-floppy-disk"></i> Lưu Mã giảm giá`;
  }

  renderTable(coupons: AdminCoupon[]) {
    if (!this.tableBody) return;
    this.tableBody.innerHTML = coupons
      .map((c) => {
        // FIX LỖI Ở ĐÂY: Nếu c.discountValue bị rỗng (undefined) thì tự động gán bằng 0
        ...
        // Ép kiểu Date an toàn, đề phòng c.expiryDate rỗng
