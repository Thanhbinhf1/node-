...
export class AdminCouponController {
  ...
  async handleEdit(id: string) {
    ...
    else alert("Lỗi tải dữ liệu mã giảm giá từ Server!");
  }
  ...
  async handleSave() {
    ...
    if (this.view.currentEditId) {
      ...
      if (success) alert("Đã cập nhật mã giảm giá!");
    } else {
      success = await this.service.addCoupon(data);
      if (success) alert("Đã tạo mã giảm giá mới!");
    }
